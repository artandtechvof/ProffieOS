...
// Play/Stop Music - hold and release while OFF/ON
...
// revert to first preset while OFF?
// OK -> Color Change mode - Twist hilt while in Colorchange mode to change blade color
// OK -> Volume Change mode - Twist hilt while in Volumechange mode to increase or decrease volume 

//
// 2 Button:
// (for double bladed sabers, POWER is always button that has the high ground...)
// (it automatically detects which button is higher or lower) 
// POWER
// OK ->   Activate - short click while OFF
// OK ->   Activate Muted - fast double click while OFF
// OK ->   Play/Stop Music - hold and release while ON/OFF
// Nice -> Turn off blade - hold and wait till blade is off while ON
// OK ->   Blaster blocks - short click while ON
// OK ->   Lockup - hold and clash while ON  (pointing up)
// OK ->   Volume UP - short click while in VOLUME MENU
// OK ->   Exit menu - hold and wait while in MENU mode		
// OKish ->Prev Preset - hold and wait while OFF
// OK ->   Color change confirm, - short click while in Colorchange mode

// AUX
// (for double bladed sabers, AUX is always button that has the low ground...)
// (it automatically detects which button is higher or lower) 
// OK -> Force Effects - short click while ON
// OK -> DRAG - hold and clash while ON (pointing down)
// Drag - hold while ON pointing the blade tip down
// OK -> Next Preset - short click while OFF
// OK -> Enter MENU/Scroll trough Menu - hold and short click POWER click while ON or OFF, 
		//Order - Menu Color, Menu Volume, Menu exit
// OK -> Exit menu - hold and wait while in MENU mode		
// OK -> Volume down - short click while in VOLUME MENU

// TWIST
// OK -> Color Change mode - Twist hilt while in Colorchange mode to change blade color
// OK -> Volume Chnage mode - Twist hilt while in Volumechnage mode to increase or decrease volume 
...
public:
  ...
  bool CfgMenu(){
	  ...
	  switch(menu_){
		  ...
		  case MENU_DIM:
			if (VolumeChangeMode()) {
				ToggleVolumeChangeMode();//Turn Volume control off
				SaveVolumeIfNeeded();
			}
  ...
  bool Event2(enum BUTTON button, EVENT event, uint32_t modifiers) override {
    ...
    // pointing up/down with hytseresis
	if (accel_.x < -0.15) {
	  pointing_down_ = true;
	} else {
	  pointing_down_ = false;	
	}
	//for hysteresis, uncomment following
	//if (accel_.x > 0.15) {
	//  pointing_down_ = false;
	//}
